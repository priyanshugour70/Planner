package com.lssgoo.planner.features.dashboard.models
...
object MotivationalThoughts {
    ...
    fun getThoughtsByCategory(category: ThoughtCategory): List<String> {
        // Safe access in case list size changed or indices are out of bounds
        // Fallback to random thoughts if indices are invalid
        return try {
             when (category) {
                ...
                ThoughtCategory.GENERAL -> if (thoughts.size > 500) thoughts.subList(500, thoughts.size) else thoughts.takeLast(10)
            }
        } catch (e: Exception) {
            listOf(getRandomThought())
        }
    }
